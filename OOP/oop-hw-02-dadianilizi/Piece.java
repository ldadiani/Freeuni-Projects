...
public class Piece {
	...
	public Piece(TPoint[] points) {
		...
		for(int i = 0; i < points.length; i++) {
			TPoint current = new TPoint(points[i]);
			body[i] = current;
			if(current.x > maxWidth) maxWidth = current.x;
			if(current.x < minWidth) minWidth = current.x;
			if(current.y > maxHeight) maxHeight = current.y;
			if(current.y < minHeight) minHeight = current.y;
		}
		...
		for(int i = 0; i < body.length; i++) {
			int currY = body[i].y;
			int currX = body[i].x;
			if(currY < skirt[currX]) skirt[currX] = currY;
		}
	...
	public Piece computeNextRotation() {
		TPoint[] rotate = new TPoint[4];
		for(int i = 0; i < body.length; i++) {
			int rotatedY = body[i].x;
			int rotatedX = height - body[i].y - 1;
			TPoint current = new TPoint(rotatedX,rotatedY);
			rotate[i] = current;
		}
	...
	public boolean equals(Object obj) {
		...
		for(int i = 0; i < this.body.length; i++) {
			...
			for(int j = 0; j < ((Piece) obj).body.length; j++) {
				if(current.equals(((Piece) obj).body[j]) && !used[j]) {
					isSame = true;
					used[j] = true;
				}
			}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece current = root;
		Piece nextr = root.computeNextRotation();
		while(!nextr.equals(root)) {
			current.next = nextr;
			current = nextr;
			nextr = current.computeNextRotation();
		}
