...
;1
(define (intersection-points circle_ls)
  (if (> 2 (length circle_ls)) '()
    (append 
      (apply append 
        (map (lambda (circle) (intersect (car circle_ls) circle))
          (cdr circle_ls)))
      (intersection-points (cdr circle_ls)))))

;2
(define (distance-product circle circle_ls)
        (if (null? circle_ls) 0
          (apply * 
            (map (lambda (second_circle) 
          			(if (equal? circle second_circle) 1
          					(dist circle second_circle))) circle_ls) )))

;3
(define (rate-points point_ls)
        (map (lambda (point) 
          (list (distance-product point point_ls) point)) 
            point_ls))
...
(define (clumped-points point_ls)
		(prefix-of-list(map cadr
      (sort-points(rate-points point_ls)))
        (quotient (length point_ls) 2)))

;6
(define (avg ls)
		(/ (apply + ls) (length ls)))
(define (average-point point_ls)
		(let ((average_point (list (avg (map car point_ls)) (avg (map cadr point_ls)))))
		(list (distance-product average_point point_ls) average_point) ))

;7
(define (best-estimate ls)
		(average-point 
      (clumped-points
       (intersection-points ls))))
