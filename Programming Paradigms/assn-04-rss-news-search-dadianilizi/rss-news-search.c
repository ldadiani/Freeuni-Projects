...
static void BuildIndices(const char *feedsFileName,  hashset * stopwords, hashset * urls, hashset * usefulwords, hashset * validwords);
static void ProcessFeed(const char *remoteDocumentName,  hashset * stopwords, hashset * urls, hashset * usefulwords, hashset * validwords);
static void PullAllNewsItems(urlconnection *urlconn,  hashset * stopwords, hashset * urls, hashset * usefulwords, hashset * validwords);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st,  hashset * stopwords, hashset * urls, hashset * usefulwords, hashset * validwords);
static void ExtractElement(streamtokenizer *st, const char *htmlTag, char dataBuffer[], int bufferLength);
static void ParseArticle(const char *articleTitle, const char *articleDescription, const char *articleURL,  hashset * stopwords, hashset * urls, hashset * usefulwords, hashset * validwords);
static void ScanArticle(streamtokenizer *st, const char *articleTitle, const char *unused, const char *articleURL, hashset * stopwords, hashset * urls, hashset * usefulwords, hashset * validwords);
...
int StrCmp(const void * void1, const void * void2){
	const char * str1=*(const char **)void1;
	const char * str2=*(const char **)void2;
	return strcasecmp(str1, str2);
}

int strHash(const void * addr, int numbuckets){
...
void freearticle(const void * addr){
  article * art=(article *)addr;
  free(art->url);
  free(art->title);
  free(art->server);
}
...
static int wordHash(const void * w, int numBuckets){
  wrd * Word=(wrd *)w;
  const char * s=(const char *)Word->word;
  int i;
  unsigned long hashcode = 0;
...
int wordCmp(const void * w1, const void * w2){
   wrd * Word1=(wrd *)w1;
   wrd * Word2=(wrd *)w2;
   const char * word1=(const char *)Word1->word;
   const char * word2=(const char *)Word2->word;
   return strcasecmp(word1, word2);
}


static int sortCmp(const void * void1, const void * void2){
	article * article1=(article *)void1;
	article * article2=(article *)void2;
	return article1->word_amount-article2->word_amount;
}
...
void wordFree(const void *addr){
	wrd * Word=(wrd *)addr;
	freefn(&Word->word);
	VectorDispose(&Word->articles);
}

int articleCmp(const void * article1, const void * article2) {
...
int main(int argc, char **argv) {
  ...
  hashset usefulwords;
  hashset validwords;
  HashSetNew(&stopwords, sizeof(char *), NumBuckets, strHash, StrCmp, freefn);
  initializeStopWords(&stopwords, stopwordsfile); 

  HashSetNew(&urls, sizeof(char *), NumBuckets, strHash, StrCmp, freefn);
  HashSetNew(&usefulwords, sizeof(wrd), NumBuckets, wordHash, wordCmp, wordFree);
  HashSetNew(&validwords, sizeof(char *), NumBuckets, strHash, StrCmp, freefn);
  Welcome(kWelcomeTextFile);
 
  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1], &stopwords, &urls, &usefulwords, &validwords);
...
static void BuildIndices(const char *feedsFileName, hashset * stopwords, hashset * urls, hashset * usefulwords, hashset * validwords) {
  ...
  while (STSkipUntil(&st, ":") != EOF) { // ignore everything up to the first selicolon of the line
    ...
    ProcessFeed(remoteFileName, stopwords, urls, usefulwords, validwords);
  }
...
static void ProcessFeedFromFile(char *fileName, hashset * stopwords, hashset *urls, hashset * usefulwords, hashset * validwords) {
  FILE *infile;
  streamtokenizer st;
  ...
  ScanArticle(&st, (const char *)fileName, articleDescription, (const char *)fileName, stopwords, urls, usefulwords, validwords);
  STDispose(&st); // remember that STDispose doesn't close the file, since STNew doesn't open one..
  fclose(infile);
...
static void ProcessFeed(const char *remoteDocumentName,  hashset * stopwords, hashset * urls, hashset * usefulwords, hashset * validwords) {
  ...
  if(!strncmp(filePrefix, remoteDocumentName, strlen(filePrefix))){
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(filePrefix), stopwords, urls, usefulwords, validwords);
    return;
  }
  ...
  switch (urlconn.responseCode) {
      case 0: printf("Unable to connect to \"%s\".  Ignoring...", u.serverName);
              break;
      case 200: PullAllNewsItems(&urlconn, stopwords, urls, usefulwords, validwords);
                break;
      case 301: 
      case 302: ProcessFeed(urlconn.newUrl, stopwords, urls, usefulwords, validwords);
                break;
...
static void PullAllNewsItems(urlconnection *urlconn,  hashset * stopwords, hashset * urls, hashset * usefulwords, hashset * validwords) {
  ...
  while (GetNextItemTag(&st)) { 
    ProcessSingleNewsItem(&st, stopwords, urls, usefulwords, validwords);
  }
...
static void ProcessSingleNewsItem(streamtokenizer *st,  hashset * stopwords, hashset * urls, hashset * usefulwords, hashset * validwords) {
  ...
  ParseArticle(articleTitle, articleDescription, articleURL, stopwords, urls, usefulwords, validwords);
...
static void ParseArticle(const char *articleTitle, const char *articleDescription, const char *articleURL,  hashset * stopwords, hashset * urls, hashset * usefulwords, hashset * validwords) {
  ...
  switch (urlconn.responseCode) {
    ...
    case 200: printf("Scanning \"%s\" from \"http://%s\"\n", articleTitle, u.serverName);
	    STNew(&st, urlconn.dataStream, kTextDelimiters, false);
		ScanArticle(&st, articleTitle, articleDescription, articleURL, stopwords, urls, usefulwords, validwords);
		STDispose(&st);
		break;
    case 301:
    case 302: 
      ParseArticle(articleTitle, articleDescription, urlconn.newUrl,  stopwords, urls, usefulwords, validwords);
		break;
 ...
 if(HashSetLookup(urls, &articleURL)!=NULL)return;
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      char * wordn1=strdup(word);
      if (WordIsWellFormed(word) && HashSetLookup(stopwords, &wordn1)==NULL){
        char * rs=(char *)HashSetLookup(validwords, &wordn1);
        if(rs==NULL){
        wrd w;
        w.word=strdup(word);
        VectorNew(&w.articles, sizeof(article), freearticle, 4); 
        HashSetEnter(usefulwords, &w); 
        wrd * strwrd=(wrd *)HashSetLookup(usefulwords, &w);
        article artic;
        artic.url=strdup(articleURL);
        artic.title=strdup(articleTitle);
        artic.server=strdup(unused);
        artic.word_amount=1;
        VectorAppend(&strwrd->articles, &artic);
        HashSetEnter(validwords, &wordn1);
        }
        else{
          wrd w;
          w.word=strdup(word);
          ...
          int index=VectorSearch(&strwrd->articles, &artt, articleCmp, 0, false);
          freefn(&w.word);
          freefn(&artt.url);
          if(index==-1){
            article artic;
            ...
            article * art=(article *)VectorNth(&strwrd->articles, index);
            art->word_amount++;
          }
          freefn(&wordn1);
        }
...
static void ProcessResponse(const char *word,  hashset * stopwords, hashset * usefulwords) {
  if (WordIsWellFormed(word)) {
	if(HashSetLookup(stopwords, &word)!=NULL){
		...
		wrd * w=(wrd *)HashSetLookup(usefulwords, &wordi);
		freefn(&wordi);
		if(w==NULL){
		printf("None of today's news articles contain the word \"%s\".\n", word);
			...
			vector v=w->articles;
			numofarticles=(((10)<(VectorLength(&v)))?(10):(VectorLength(&v)));
			VectorSort(&v, sortCmp);
			int i;
			for(i=numofarticles-1; i>=0; --i){
				article * art=VectorNth(&v, i);
		
				if(art->word_amount==1){
					printf("%d.) \"%s\" [search term occurs %d time]\n\"%s\"\n", numofarticles-i, art->title, art->word_amount, art->url);
				}
				else{
